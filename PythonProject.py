...
path='C:/ComputerProgrammingProject_main-folder'
print('-'*50)
...
def createFile(nm,lst):
    with open(f'{path}/{nm}','a',newline='')as f:
        script= csv.writer(f)
        script.writerow(lst)
        print(f"{nm} file has been UPDATED")

def percent(number):
    if stream.lower()=='cse' or stream.lower()=='cseai' or stream.lower()=='cseaiml' or stream.lower()=='cseiotcsbs':
        number=(number*100)//600
...
def grade(n):
    if n>=90:
        ...
        return("You have Failed the Exam and got grade F.")
...
def getBatch():
    with open(f'C:/ComputerProgrammingProject_main-folder/Batch.csv','r') as f:
        reader=csv.reader(f)
...
def remove(s):
    with open(f'C:/ComputerProgrammingProject_main-folder/Student.csv','r+',newline='') as f:
        script=csv.reader(f)
...
def graph_Of_Courses():
    ...
    with open(f'{path}/Course.csv','r')as f:
        ...
        for k in range(6):
            no_of_A=lst[k][1].count('A')
            no_of_B=lst[k][1].count('B')
            no_of_C=lst[k][1].count('C')
            no_of_D=lst[k][1].count('D')
            no_of_E=lst[k][1].count('E')
            no_of_F=lst[k][1].count('F')
            lst[k][1]={'A':no_of_A,'B':no_of_B,'C':no_of_C,'D':no_of_D,'E':no_of_E,'F':no_of_F}
...
def departmentGraph():
    ...
    x_axis = list(need.keys())
    y_axis = list(need.values())
    color_lst=['#C70039','#9BB1F2','#FFC300','#FF5733','#DAAFB1','#86B7C8']
    fig,ax = plt.subplots()
    ax.set_facecolor("Black")
    fig.set_facecolor("Black")
    ...
    plt.barh(x_axis,y_axis,color=color_lst,height=0.3,align='center')
...
while msg:
    ...
    with open(f'{path}/Course.csv','a',newline='')as f:
        script= csv.writer(f)
        script.writerow(['C001','Computer Programming'])
...
for i in range(no_of_students):
    ...
    if duplicate('Student.csv',student_id,0):
        ...
        print()
        print("Subjects are like [Computer,Math,Physics,Chemistry,Biology,English]")
        print('Enter the subjects marks out of 100. In the same order which is mentioned  (e.g. [100,100,82,75,69,85])')
        print()
        marks_lst=eval(input("Enter the Marks as list : "))
        ...
        with open(f"{path}/ReportCards/{student_id}_{''.join(name.split())}.txt",'w') as f:
            ...
            f.writelines([f'Name of the student : {name} \n',
                          ...
                          '\n',
                          f'Marks in Math is : {marks_lst[1]} \n',
                          f'Marks in Computer is : {marks_lst[0]} \n',
                          f'Marks in Physics is : {marks_lst[2]} \n',
                          f'Marks in Chemistry is : {marks_lst[3]} \n',
                          f'Marks in Biology is : {marks_lst[4]} \n',
                          f'Marks in English is : {marks_lst[5]} \n'])
...
try:            
    ...
    print("Nothing is added in Department.csv")
...
print('Please Close the window after viewing to continue')
screenLoad()
graph_Of_Courses()
print()
print()
